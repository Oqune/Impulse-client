...
- **Экспорт/импорт ключей:** криптографический экспорт через Argon2id + AES-256-GCM для переноса ML-KEM идентичности между устройствами (новый ML-DSA ключ генерируется при импорте).

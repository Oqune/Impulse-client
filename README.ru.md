...
## Что изменилось в v2.5

Исправления доставки и отображения сообщений:

- **Порядок сообщений** — сообщения теперь отображаются в правильном хронологическом порядке (сортировка по серверному timestamp, а не по temp ID).
- **Дедуп оптимистичных** — подтверждённые сообщения корректно заменяют свои оптимистичные копии (content-based сопоставление).
- **Последовательная обработка sync** — `onSyncResponse` и `processPendingMessages` теперь обрабатывают сообщения последовательно для сохранения порядка.
- **Атомарность записи** — `st.write(buf)` retry захватывает фактически записанные байты; ошибки записи больше не молча теряют данные.
- **Устойчивость сервера** — неизвестные опкоды пропускаются вместо разрыва сессии; гонки в хранилище исправлены правильными блокировками.
...
|---------|------------------|------|
| Транспорт | `WebSocket` (OkHttp, `wss://`) | **`WebTransport`** (Android `android.net.http`, API 28+, HTTPS/QUIC) |
| Формат кадров | JSON с разделителями-новыми-строками | **Бинарный протокол** (опкоды `0x01`–`0x0C`, кадры с длиной-префиксом) |
| Аутентиф. сервера | доверять любому самоподписанному | **Привязка сертификата** через `serverCertificateHashes` (TOFU) |
| Обмен ключами | нет / статический ключ | **ML-KEM-768** (KEM) + **ML-DSA-65** подписи отправителя |
| Групповой секрет | KEM инкапсуляция | **Per-recipient KEM**: каждое сообщение зашифровано N раз (на каждого получателя) через ML-KEM-768 инкапсуляцию; обёрнутые ключи + шифротекст в одном `OP_DATA` |

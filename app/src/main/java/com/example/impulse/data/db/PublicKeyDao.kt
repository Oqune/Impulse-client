...
interface PublicKeyDao {
    ...
    @Query("SELECT kem_public_key FROM public_keys WHERE server_id = :serverId AND fingerprint = :fingerprint AND kem_public_key IS NOT NULL")
    suspend fun getKemKey(serverId: String, fingerprint: String): ByteArray?

    @Query("SELECT * FROM public_keys WHERE server_id = :serverId AND kem_public_key IS NOT NULL")
    suspend fun getAllKeys(serverId: String): List<PublicKeyEntity>

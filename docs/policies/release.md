...
## 5. Владелец подписи
Локальный агент на ПК (keystore не покидает ПК — требование
`decisions/agent-federation.md` и `decisions/impulse-ai-dev-setup.md`).
Вопрос «кто делает релизы — агент или вручную» открыт в OKF; фактически —
агент локально на ПК.
